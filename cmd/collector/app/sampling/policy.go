...
type TraceData struct {
	...
	ReceivedBatches []Spans
}

// Spans stores the sampling related trace data.
type Spans struct {
	Spans []*model.Span
}
...
type PolicyEvaluator interface {
	...
	OnLateArrivingSpans(earlyDecision Decision, spans []*model.Span) error

	// Evaluate looks at the trace data and returns a corresponding SamplingDecision.
	Evaluate(trace *TraceData) (Decision, error)

	// OnDroppedSpans is called when the trace needs to be dropped, due to memory
	// pressure, before the decision_wait time has been reached.
	OnDroppedSpans(trace *TraceData) (Decision, error)
}
